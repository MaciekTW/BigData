...
val query=streamingCountsDF.writeStream.outputMode("complete").format("memory").queryName("query").start()
...
// MAGIC %sql select action, date_format(window.end, "MMM-dd HH:mm") as time, count from query order by time, action
